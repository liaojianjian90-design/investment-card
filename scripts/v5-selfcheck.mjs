...
assert(serviceWorkerSource.includes("investment-card-github-pages-v522"), "Service Worker 缓存版本必须升级到 v522");
...
assert((rules.dipBuyRules || []).find((rule) => rule.id === "dip-btc-58000")?.enabled === false, "BTC 58000 档必须在 5.3.5.1 中禁用");
assert((rules.watchBuyRules || []).find((rule) => rule.id === "watch-xaut-3850")?.amount === 500, "XAUT 3850 档应调整为 500 USDT");

console.log(JSON.stringify({ ok: true, score: score.total, grade: score.grade, themeLayer: v.themeLayer.name, version: "5.3.5.1", holdingsMode: "holdings-json-only" }, null, 2));
