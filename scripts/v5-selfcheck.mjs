...
assert(Math.abs(v.themeLayer.targetMax - 0.20) < 1e-9, "AI抽水机仓成熟目标上限应为 20%");
assert(Math.abs(v.themeLayer.hardMax - 0.25) < 1e-9, "AI抽水机仓硬上限应为 25%");
...
assert(holdings.positions.some((item) => item.symbol === "RAM"), "holdings.json 必须包含 RAM 短线T仓");
...
setWeight(highTheme, "MU", 10);
setWeight(highTheme, "DRAM", 8);
setWeight(highTheme, "GLW", 8);
...
assert(serviceWorkerSource.includes("investment-card-github-pages-v514"), "Service Worker 缓存版本必须升级到 v513");

console.log(JSON.stringify({ ok: true, score: score.total, grade: score.grade, themeLayer: v.themeLayer.name, holdingsMode: "holdings-json-only" }, null, 2));
