...
5.2.2 进一步强化“有效仓位”原则：核心仓、稳定仓、AI 主攻仓不再发出 100 USDT 级别买入提醒；BTC/ETH、VOO/XAUT、MU/DRAM/GLW/SMH 等主线单笔建议提升到 500 USDT 级别，避免看对行情但赚不到钱。
...
5.2.2 继续将 `GLW` 正式放在第一主攻层，与 `MU`、`DRAM` 同等重视；同时提高有效买入金额，避免 GLW 只停留在心理仓。
...
## 5.2.2 新增：AI主攻仓急跌机会捕捉

5.2.2 把“现金很高、AI主攻仓不足、MU/DRAM/GLW/SMH 盘中急跌”写成邮件触发规则，避免出现主攻标的盘中大跌后快速拉回，但系统没有提醒、用户只补几百美金的情况。
...
- AI主攻仓未达到 20% 目标；
- 当天没有主动交易；
- 触发标的属于 MU / DRAM / GLW / SMH 等第一主攻层，或 SMH 半导体篮子确认。

机会分级：

| 级别 | 条件 | 邮件建议 |
|---|---|---|
| A级 | 任一主攻标的当前跌幅 ≥5%，或两个以上主攻标的同时跌幅 ≥4% | 1000-1500 USDT 有效买入 |
| S级 | 现金 ≥55%、AI仓 <15%、三个以上主攻标的同时急跌，SMH 跌幅 ≥3% | 1500-2500 USDT 篮子买入 |
| 复盘 | 日内最大跌幅 ≥5%，随后从低点反弹 ≥4% | 只做错过机会复盘，不追高 |

风控边界：

- 单日 AI 主攻仓买入不超过总资产 12%；
- 单一主攻标的单日买入通常不超过 1000 USDT；
- 2x DRAM/RAM 单日不超过 500 USDT；
- AI 总仓超过 30% 后停止新增；
- 财报暴雷、公司基本面恶化、数据过期时不触发急跌买入邮件。
