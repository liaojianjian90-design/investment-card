...
- 手机端保持纵向卡片，不使用横向滑动卡片；通过压缩间距和双列摘要降低页面长度。
...
- 价格源顺序调整为：优先 Bitget。MRVL/ANET/TSM/ASML/SMH/SOXX 配置为 Bitget 候选交易对优先尝试，部分标的同时尝试 R 前缀与无 R 前缀交易对；Bitget 无可用价格时再尝试 Yahoo Finance 与 Stooq。
